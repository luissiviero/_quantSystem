...
import { useTicker } from '../hooks/useBinanceData';

const Header = React.memo(() => {
  const { price, change, mark } = useTicker('BTCUSDC');
  const changeColor = parseFloat(change) >= 0 ? '#0ecb81' : '#f6465d';
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', fontSize: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
             ...
             <span style={{ color: '#eaecef' }}>{mark}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
